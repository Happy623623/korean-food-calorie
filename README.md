...
**✅ 다중 검출 성공** — 피자 + 비빔밥이 별개 박스로 잡혀 다중 경로로 분기, 항목별 칼로리를 합산한다.

![다중 검출 성공 (피자+비빔밥 904 kcal)](screenshots/multi_success.webp)

> 박스 2개(피자·비빔밥) → `multi` 분기 → 피자 265 + 비빔밥 639 = **총 904 kcal**.
> 두 음식이 화면에서 뚜렷이 분리돼 있어 검출기가 각각을 잘 잡은 경우다.

**⚠️ 한계 — 식판(여러 칸) 사진은 다중 검출에 실패**한다. 실제 식판 사진(비빔밥·국·반찬 여러 칸)을
넣으면 검출기가 **비빔밥 그릇 1개만**(conf 0.92) 잡고 나머지(특히 국그릇)는 **conf 0.05 까지 낮춰도
박스를 제안하지 않아**, 박스 ≤1개 → `single` 로 분기한다.

![식판 다중 검출 한계 (비빔밥 1박스만 검출)](screenshots/limitation_tray.jpg)

> 같은 식판의 국그릇·반찬은 conf 0.05 에서도 박스가 잡히지 않아(초록 박스 1개뿐) `single` 로 분기된다.

| 구분 | 성공 사례(위) | 한계 사례(식판) |
|---|---|---|
| 입력 | 피자 + 비빔밥(분리 배치) | 식판 5칸(비빔밥·국·반찬·음료) |
| 검출 박스(conf 0.05) | 2개 | **1개**(비빔밥만) |
| 분기 | `multi` (904 kcal 합산) | `single` (전체 이미지 1종) |

원인은 conf 임계값이 아니라 **검출기 recall**이다. 검출기 학습 데이터가 *이미지당 단일·중앙 음식*
중심이라, 식판처럼 칸이 나뉜 배치에서는 가장 지배적인 한 음식만 잡고 나머지를 제안하지 못한다.
근본 해결은 **다중 음식/식판 이미지로 검출기를 재학습**하는 것이다(아래 [검출기 한계](#한계-정직한-기술-1) 참고).
